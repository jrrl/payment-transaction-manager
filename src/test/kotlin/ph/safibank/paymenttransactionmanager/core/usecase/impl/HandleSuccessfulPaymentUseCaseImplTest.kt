...
class HandleSuccessfulPaymentUseCaseImplTest {
    private val transactionRepo = TransactionRepoMockImpl()
    private val eventService: EventService = mockk()
    private val postingService: PostingService = mockk()

    private val useCase = HandleSuccessfulPaymentUseCaseImpl(transactionRepo, eventService, postingService)
    ...
    @BeforeEach
    fun setup() {
        coJustRun { eventService.sendTransactionPendingSettlementEvent(any()) }
        coEvery { postingService.settleAmount(any()) } returns PostingResult(UUID.randomUUID())
    }
    ...
    @Test
    fun `should update transaction status to PENDING_SETTLEMENT`() = runTest {
        transactionRepo.saveTransaction(transaction)

        useCase.invoke(transaction.id)

        expectThat(transactionRepo.getTransaction(transaction.id)).isNotNull().and {
            get { status } isEqualTo TransactionStatus.PENDING_SETTLEMENT
        }
    }

    @Test
    fun `should update transaction provider status to SUCCESS`() = runTest {
        transactionRepo.saveTransaction(transaction)

        useCase.invoke(transaction.id)

        expectThat(transactionRepo.getTransaction(transaction.id)).isNotNull().and {
            get { providerDetails.providerStatus } isEqualTo ProviderStatus.SUCCESS
        }
    }
    ...
    fun `should update transaction posting details`() = runTest {
        ...
        expectThat(transactionRepo.getTransaction(transaction.id)).isNotNull().and {
            get { postingDetails }.isNotNull().and {
                get { status } isEqualTo PostingStatus.PENDING_SETTLEMENT
    ...
    @Test
    fun `should settle posting details`() = runTest {
        transactionRepo.saveTransaction(transaction)
        ...
        coVerify(exactly = 1) {
            postingService.settleAmount(capture(capturedTransactionEvent))
        }
    ...
    @Test
    fun `should send transaction success event`() = runTest {
        transactionRepo.saveTransaction(transaction)

        useCase.invoke(transaction.id)

        coVerify(exactly = 1) { eventService.sendTransactionPendingSettlementEvent(any()) }
    }

    @Test
    fun `should not update transaction when settleAmount event fails`() = runTest {
        coEvery { postingService.settleAmount(any()) } throws InfraException("", RuntimeException())
        transactionRepo.saveTransaction(transaction)

        expectCatching {
            useCase.invoke(transaction.id)
        }.isFailure().isA<InfraException>()
        expectThat(transactionRepo.getTransaction(transaction.id)) isEqualTo transaction
    }

    @Test
    fun `should update transaction even when sendTransactionPendingSettlementEvent fails`() = runTest {
        coEvery { eventService.sendTransactionPendingSettlementEvent(any()) } throws InfraException("", RuntimeException())
        transactionRepo.saveTransaction(transaction)
